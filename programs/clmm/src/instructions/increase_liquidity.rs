...
use crate::{
    errors::ClmmError, instructions::transfer_tokens, states::{Pool, Position, TickArrayState}, utils::{get_amounts_for_liquidity, tick_to_sqrt_price_x96}
};
...
tick_array_upper_start_index: i32
  ...
  let position = &mut ctx.accounts.position;
   ...
   position.liquidity =  position.liquidity.checked_add(liquidity_amount).ok_or(ClmmError::ArithmeticOverflow)?;
